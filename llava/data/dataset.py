...
class LazyVLNCEDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        media_payload, question, answer, token_types, video_ok = self._build_sample(sample)
        conversation = [
            {"from": "human", "value": question},
            {"from": "gpt", "value": answer},
        ]
        ...
        data_dict = preprocess(sources, self.tokenizer, has_image=media_payload is not None)
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0])

        data_dict["image"] = media_payload
        data_dict["mm_token_types"] = token_types
        if media_payload is not None and not video_ok:
            data_dict["labels"][:] = IGNORE_INDEX
        return data_dict
...
class DataCollatorForSupervisedDataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            ...
            payload = instance.get("image")
            if payload is not None and not isinstance(payload, dict):
                payload = {"rgb": payload}
            payloads.append(payload)
            token_hints.append(instance.get("mm_token_types", []))
        ...
        packed = self._pack_modalities(payloads, token_hints, input_ids)
        if packed is None:
            batch["images"] = None
        else:
            batch["images"] = packed
    ...
    def _pack_modalities(self, payloads, token_hints, input_ids):
        flattened = {"rgb": [], "depth": [], "point": []}
        token_types = []
        for payload, hints, ids in zip(payloads, token_hints, input_ids):
            num_tokens = (ids == IMAGE_TOKEN_INDEX).sum().item()
            if num_tokens == 0 or payload is None:
                continue
            inferred = hints or self._infer_tokens(payload)
            inferred = inferred[:num_tokens]
            token_types.extend(inferred)
            offsets = {"rgb": 0, "depth": 0, "point": 0}
            for token in inferred:
                tensor = payload.get(token)
                if tensor is None:
                    raise ValueError(f"Missing tensor for modality token '{token}'.")
                if token in {"rgb", "depth"} and tensor.dim() == 3:
                    tensor = tensor.unsqueeze(0)
                chunk = tensor[offsets[token]]
                flattened[token].append(chunk)
                offsets[token] += 1
        if not token_types:
            return None
        packed = {"token_types": token_types}
        for key, tensors in flattened.items():
            if tensors:
                packed[key] = torch.stack(tensors, dim=0)
        return packed

    def _infer_tokens(self, payload: Dict[str, torch.Tensor]) -> List[str]:
        tokens = []
        for modality in ("rgb", "depth", "point"):
            tensor = payload.get(modality)
            if tensor is None:
                continue
            count = tensor.shape[0] if tensor.dim() > 2 else 1
            tokens.extend([modality] * count)
        return tokens
