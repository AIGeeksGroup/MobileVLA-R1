...
from llava.model.multimodal_encoder.builder import build_depth_tower, build_point_tower, build_vision_tower
...
class LlavaMetaModel(ABC):
    def init_vlm(self, config: PreTrainedModel = None, *args, **kwargs):
        ...
        self.vision_tower = build_vision_tower(vision_tower_cfg, config)
        self.mm_projector = build_mm_projector(mm_projector_cfg, config)
        self.depth_tower = build_depth_tower(getattr(config, "depth_tower_cfg", None), config)
        ...
        if self.point_tower is not None and getattr(config, "point_hidden_size", mm_hidden) != mm_hidden:
            self.point_bridge = nn.Linear(config.point_hidden_size, mm_hidden)
        model_dtype = eval(config.model_dtype)
        if self.depth_tower is not None:
            self.depth_tower = self.depth_tower.to(model_dtype)
            if self.depth_bridge is not None:
                self.depth_bridge = self.depth_bridge.to(model_dtype)
        if self.point_tower is not None:
            self.point_tower = self.point_tower.to(model_dtype)
    ...
    def post_config(self):
        ...
        if getattr(self.config, "mm_projector_cfg", None) is None:
            self.config.mm_projector_cfg = self.mm_projector.config
        if getattr(self.config, "depth_tower_cfg", None) is None and getattr(self, "depth_tower", None):
            self.config.depth_tower_cfg = getattr(self.config, "depth_tower_cfg", "depth_encoder")
        if getattr(self.config, "point_tower_cfg", None) is None and getattr(self, "point_tower", None):
            self.config.point_tower_cfg = getattr(self.config, "point_tower_cfg", "point_encoder")
    ...
    def freezed_module_patch(self):
        ...
        if getattr(self, "depth_tower", None) and not getattr(self.config, "tune_depth_tower", False):
            self.depth_tower.eval()
        if getattr(self, "point_tower", None) and not getattr(self.config, "tune_point_tower", False):
            self.point_tower.eval()

    def encode_images(self, images):
        if isinstance(images, dict):
            return self._encode_multi_modal(images)
        image_features = self.get_vision_tower()(images)
        image_features = self.get_mm_projector()(image_features)
        return image_features

    def _encode_multi_modal(self, payload: dict):
        token_types = payload.get("token_types", [])
        if len(token_types) == 0:
            return None

        device = self.device
        dtype = eval(self.config.model_dtype)

        buffers = {}
        rgb_inputs = payload.get("rgb")
        if rgb_inputs is not None:
            rgb_features = self.get_vision_tower()(rgb_inputs.to(device=device, dtype=dtype))
            rgb_features = self.get_mm_projector()(rgb_features)
            buffers["rgb"] = rgb_features

        depth_inputs = payload.get("depth")
        if depth_inputs is not None and self.depth_tower is not None:
            depth_feats = self.depth_tower(depth_inputs.to(device=device, dtype=dtype))
            if self.depth_bridge is not None:
                depth_feats = self.depth_bridge(depth_feats)
            buffers["depth"] = self.get_mm_projector()(depth_feats)

        point_inputs = payload.get("point")
        if point_inputs is not None and self.point_tower is not None:
            point_feats = self.point_tower(point_inputs.to(device=device, dtype=dtype))
            if self.point_bridge is not None:
                point_feats = self.point_bridge(point_feats)
            buffers["point"] = self.get_mm_projector()(point_feats)

        assembled = []
        counters = {"rgb": 0, "depth": 0, "point": 0}
        for token in token_types:
            if token not in buffers:
                raise ValueError(f"Missing tensor buffer for modality token: {token}")
            idx = counters[token]
