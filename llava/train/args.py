...
class DataArguments:
    ...
    # NavCoT-specific multi-modal assets
    navcot_image_root: Optional[str] = field(
        default=None,
        metadata={"help": "Override root directory for Nav-CoT RGB frames (e.g., MP3D scans)."},
    )
    navcot_depth_root: Optional[str] = field(
        default=None,
        metadata={"help": "Directory that stores Depth Anything v2 depth maps aligned with Nav-CoT frames."},
    )
    navcot_depth_format: str = field(
        default="png",
        metadata={"help": "Depth file format for Nav-CoT samples. Supports png, npy, pt."},
    )
    navcot_depth_scale: float = field(
        default=1000.0,
        metadata={"help": "Scaling divisor applied to integer depth maps (e.g., 1000 for millimeters)."},
    )
    navcot_use_depth: bool = field(
        default=False,
        metadata={"help": "Whether to load depth maps (produced by Depth Anything v2) for Nav-CoT samples."},
    )
    navcot_use_point: bool = field(
        default=False,
        metadata={"help": "Whether to derive point clouds (encoded by Point Transformer) for Nav-CoT samples."},
    )
    navcot_pointcloud_points: int = field(
        default=2048,
        metadata={"help": "Number of points to keep per Nav-CoT sample when deriving point clouds."},
    )
    navcot_point_normalize: bool = field(
        default=True,
        metadata={"help": "If True, normalize point clouds to zero-mean unit variance before encoding."},
    )
    navcot_depth_frames: int = field(
        default=1,
...
class ModelArguments:
    ...
    depth_tower: Optional[str] = field(
        ...
        metadata={"help": "Depth encoder identifier or path when --use_depth_tower is enabled."},
    )
    point_tower: Optional[str] = field(
        default="point_transformer",
        metadata={"help": "Point-cloud encoder identifier or path when --use_point_tower is enabled."},
    )
    depth_hidden_size: int = field(
        default=1024,
        metadata={"help": "Hidden size of depth encoder outputs before the multimodal projector."},
    )
    point_hidden_size: int = field(
        default=1024,
        metadata={"help": "Hidden size of point-cloud encoder outputs before the multimodal projector."},
    )
    pointcloud_points: int = field(
        default=2048,
        metadata={"help": "Number of points the point encoder expects (should match data arguments)."},
    )
